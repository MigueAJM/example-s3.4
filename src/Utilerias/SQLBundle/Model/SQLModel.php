...
class SQLModel
{
    ...
    public function selectFromTable($table, $fields = array(), $where = array(), $order = array(), $joins = [])
    {
        $this->buildColumnsWithAS($fields);
        $bool = $this->wordSearch($fields, ' as ');
        $qry = 'SELECT ';
        $qry .= $bool ? $this->buildColumnsWithAS($fields) : $this->buildSelectFields($fields);
        $qry .= ' FROM';
        $qry .= ' "' . $this->schema . '"."' . $table . '" ';
        if (count($joins))
            $qry .= $this->buildJoins($joins);
    ...
    /**
     * Obtienen las columnas concatenadas.
     * @return String columnas concatenadas y separadas por comas.
     */
    private function buildColumnsWithAS(array $columns): String
    {
        $query = '';
    ...
    /**
     * Busqueda de una palabra(insensible a mayusculs o minusculas) en un array de string,
     * $array => array de string a consultar,
     * $word => palabra a buscar.
     * @return Bool true or false
     */
    public function wordSearch(array $array = [], String $word = ''): Bool
    {
        $bool = 0;
        $coincidence = 0;
        if (is_array($array) && is_string($word)) {
            foreach ($array as $key => $string) {
                if (stristr($string, $word)) {
                    $coincidence++;
                }
            }
            $bool = $coincidence ? 1 : 0;
        }
        return $bool;
    }

    public function buildJoins(array $joins)
    {
        $query = '';
        if (is_array($joins) && count($joins)) {
            foreach ($joins as $key => $value) {
                ...
                $query .= is_array($value) ? ' ' . $value['type'] . ' JOIN ' . $key . ' ON ' . $value['key'] : ' INNER JOIN ' . $key . ' ON ' . $value;
            }
        }
        return $query;
    }
